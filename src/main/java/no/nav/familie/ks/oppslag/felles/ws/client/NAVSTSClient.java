...
import no.nav.familie.ks.oppslag.felles.ws.PropertyUtil;
import no.nav.familie.ks.oppslag.felles.ws.sikkerhet.context.SubjectHandler;
import no.nav.familie.ks.oppslag.felles.ws.sikkerhet.domene.IdentType;
import no.nav.familie.ks.oppslag.felles.ws.sikkerhet.domene.SluttBruker;
...
public class NAVSTSClient extends STSClient {
    ...
    public SecurityToken requestSecurityToken(String appliesTo, String action, String requestType, String binaryExchange) throws Exception {
        ...
        if (key == null) {
            throw new RuntimeException("kan Ikke Hente Saml Uten Oidc Token");
        }
